...
#define _PIN_LED_29_  29
#define _PIN_LED_30_  30
...
unsigned int LEDTimerCounter = 0;
void doSomething()
{
  if (TimerCounter > 0)TimerCounter--;
  if (LEDTimerCounter > 0)LEDTimerCounter--;
}

//For HCSR04
UltraSonicDistanceSensor distanceSensor(_PIN_6_, _PIN_3_);

void setup() {
  t.every(100, doSomething);
  Serial.begin(9600);
  ...
  pinMode(_PIN_LED_29_, OUTPUT);
  pinMode(_PIN_LED_30_, OUTPUT);
}
...
void loop() {
  ...
  //For LED flash 1000ms once
  if (LEDTimerCounter == 0)
  {
    LEDTimerCounter = 10;
    if (LED_Blink == 1)
    {
      LED_Blink = 0;
      digitalWrite(_PIN_LED_29_, HIGH);
      digitalWrite(_PIN_LED_30_, LOW);
    }
    else
    {
      LED_Blink = 1;
      digitalWrite(_PIN_LED_29_, LOW);
      digitalWrite(_PIN_LED_30_, HIGH);
    }
  }
