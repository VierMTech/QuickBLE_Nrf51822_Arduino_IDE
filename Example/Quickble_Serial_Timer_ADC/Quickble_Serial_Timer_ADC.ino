...
void loop() {
  ...
  if (TimerCounter == 0)
  {
    TimerCounter = 10;  //100ms per unit
    
    PIN4_ADC = analogRead(A0);
    Serial.print("ADC A0 PIN4: ");  //QuickBLE PIN4 (range:0~3.3V)
    Serial.print(PIN4_ADC);         //0~1024
    Serial.print(", Voltage(0~3.3V):");
    Serial.print(PIN4_ADC*3.3/1024);          //0~3.3
    Serial.println("V");
